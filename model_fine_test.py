...
FINE_PATH = '/home/iffly/dataset/finemodel/finetuning_ucf101.model'
if __name__ == '__main__':
    with tf.Graph().as_default():
        ...
        net = C3d(num_class=20).build_model()
        sess = tf.Session()
        init = tf.global_variables_initializer()
        sess.run(init)
        variables_to_restore = slim.get_variables_to_restore(exclude=['var_name/wd1', 'var_name/wd2', 'var_name/wout',
                                                                      'var_name/bd1', 'var_name/bd2', 'var_name/bout'])

        saver = tf.train.Saver(variables_to_restore)

        saver.restore(sess, "/home/iffly/dataset/sports1m_finetuning_ucf101.model")
        saver = tf.train.Saver()
        saver.save(sess, "/home/iffly/dataset/finemodel1/finetuning_ucf101.model")
